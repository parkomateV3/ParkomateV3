...
class DataByDay extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'databyday:cron';
     ...
     * @var string
     */
    protected $description = 'Data By Day';
    ...
    public function handle()
    {
        ...
        foreach ($SiteData as $siteId => $entries) {
            ...
            if (!empty($groupedData)) {
                ...
                foreach ($groupedData as $sensorId => $statuses) {
                    ...
                    if ($firstStatus == 1) {
                        $prevZero = DB::table('site_data_by_minutes')
                            ->whereRaw("JSON_UNQUOTE(JSON_EXTRACT(data, '$.\"{$sensorId}\"')) = '0'")
                            ->whereDate('created_at', '<', $date)
                            ->orderBy('created_at', 'desc')
                            ->first();

                        $siteFirstData = DB::table('site_data_by_minutes')
                            ->where('site_id', $siteId)
                            ->whereDate('created_at', $date)
                            ->orderBy('created_at', 'desc')
                            ->first();
                        if ($prevZero) {
                            $initialCount = Carbon::parse($siteFirstData->created_at)
            ...
            $dataStore = [
                ...
                'max_time' => $maxTime,
                'avg_time' => $avgTime,
                'expected_amount' => empty($prices) ? 0 : array_sum($prices),
            ];
            site_data_by_day::create($dataStore);
        }
        ...
        foreach ($FloorData as $floorId => $entries) {
            ...
            $dataStore = [
                ...
                'max_time' => $maxTime,
                'avg_time' => $avgTime,
                'expected_amount' => array_sum($prices),
            ];
            floor_data_by_day::create($dataStore);
        }
    }
}
