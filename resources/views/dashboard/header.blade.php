...
<style>
  @media (max-width: 767.98px) {
    ...
    #live-status-refresh {
      order: 3;
      /* set to a higher number to push it to the end */
    }
    ...
    <div class="sidebar-wrapper group">
      ...
      <div class="sidebar-menus bg-white dark:bg-slate-800 py-2 px-4 h-[calc(100%-80px)] overflow-y-auto z-50"
        ...
        <ul class="sidebar-menu">
          ...
          $site_type = getSiteType($site_id);

          $access = Auth::user()->access;
          $accessArray = explode(',', $access);
          @endphp
          @if($site_type != 'slot_reservation' && $site_type != 'eecs')
          @php
          ...
          @endphp
          @if($flagfloor == 0)
          <a href="#" class="paddingfloormap toolTip onTop onClickTooltip navItem {{$active == 'floor-map' ? 'active' : ''}}" data-tippy-content="No Access" data-tippy-theme="danger">
            <span class="flex items-center">
              <iconify-icon class="nav-icon" icon="bx:file"></iconify-icon>
              <span>Floor Map</span>
            </span>
          </a>
          @else
          <li class="{{$active == 'floor-map' ? 'active' : ''}}">
            ...
            <ul class="sidebar-submenu {{$active == 'floor-map' ? 'menu-open' : ''}}">
              @php
              $floorData = getFloorsData();
              @endphp
              @foreach($floorData as $floor)
              <li class="navItem {{$active == 'floor-map' && $floor->floor_id == $floor_id ? 'active' : ''}}">
                <a href="{{ url('dashboard/floormap', $floor->floor_id) }}" class="{{$active == 'floor-map' && $floor->floor_id == $floor_id ? 'color-white' : 'color-black'}}">{{ $floor->floor_name }}</a>
              </li>
              @endforeach
            </ul>
          </li>
          ...
          <li>
            ...
            <a href="#" class="toolTip onTop onClickTooltip navItem {{$active == 'table-view' ? 'active' : ''}}" data-tippy-content="No Access" data-tippy-theme="danger">
              ...
              <a href="{{ route('dashboard/table-view') }}" class="navItem {{$active == 'table-view' ? 'active' : ''}}">
                @endif
                <span class="flex items-center">
                  <iconify-icon class=" nav-icon" icon="material-symbols:table-view"></iconify-icon>
          ...
          <li>
            ...
            <a href="#" class="toolTip onTop onClickTooltip navItem {{$active == 'detailed-view' ? 'active' : ''}}" data-tippy-content="No Access" data-tippy-theme="danger">
              ...
              <a href="{{ route('dashboard/detailed-view') }}" class="navItem {{$active == 'detailed-view' ? 'active' : ''}}">
                @endif
                <span class="flex items-center">
                  <iconify-icon class=" nav-icon" icon="fluent:apps-list-detail-20-filled"></iconify-icon>
          ...
          <li>
            ...
            <a href="#" class="toolTip onTop onClickTooltip navItem {{$active == 'history' ? 'active' : ''}}" data-tippy-content="No Access" data-tippy-theme="danger">
              ...
              <a href="{{ route('dashboard/history') }}" class="navItem {{$active == 'history' ? 'active' : ''}}">
                @endif
                <span class="flex items-center">
                  <iconify-icon class="nav-icon" icon="icon-park-outline:database-time"></iconify-icon>
          ...
          <li>
            ...
            <a href="#" class="toolTip onTop onClickTooltip navItem {{$active == 'summary-report' ? 'active' : ''}}" data-tippy-content="No Access" data-tippy-theme="danger">
              ...
              <a href="{{ route('dashboard/summary-report') }}" class="navItem {{$active == 'summary-report' ? 'active' : ''}}">
                @endif
                <span class="flex items-center">
                  <iconify-icon class="nav-icon" icon="tabler:report"></iconify-icon>
          ...
          <li>
            <a href="{{ url('dashboard/findmycar') }}" class="navItem {{$active == 'findmycar' ? 'active' : ''}}">
              <span class="flex items-center">
                <iconify-icon class="nav-icon" icon="mdi:car-find"></iconify-icon>
                <span>Find My Car</span>
              </span>
            </a>
          </li>
          @endif
          @if($site_type == 'slot_reservation')
          @php $siteFloorData = getSiteFloorData($site_id); @endphp
          <li class="{{$active == 'reservations' ? 'active' : ''}}">
            <a href="javascript:void(0)" class="navItem">
              <span class="flex items-center">
                <iconify-icon class=" nav-icon" icon="heroicons:squares-plus-20-solid"></iconify-icon>
                <span>Reservations</span>
              </span>
              <iconify-icon class="icon-arrow" icon="heroicons-outline:chevron-right"></iconify-icon>
            </a>
            <ul class="sidebar-submenu {{$active == 'reservations' ? 'menu-open' : ''}}">
              @foreach($siteFloorData as $floor)
              <li class="navItem {{$active == 'reservations' && $floor->floor_id == $floor_id ? 'active' : ''}}">
                <a href="{{ route('dashboard/reservations', $floor->floor_id) }}" class="{{$active == 'reservations' && $floor->floor_id == $floor_id ? 'color-white' : 'color-black'}}">{{ $floor->floor_name }}</a>
              </li>
              @endforeach
            </ul>
          </li>
    ...
    <div class="flex flex-col justify-between">
          ...
          <div
            ...
            <div class="flex justify-between items-center h-full">
              <div class="flex items-center md:space-x-4 space-x-2 xl:space-x-0 rtl:space-x-reverse vertical-box">
                <button class="smallDeviceMenuController block leading-0">
                  <iconify-icon class="cursor-pointer text-slate-900 dark:text-white text-2xl"
                    icon="heroicons-outline:menu-alt-2"></iconify-icon>
                </button>
      ...
      overflow-hidden">
            ...
            dark:text-white font-normal">
                ...
                <!-- END: Header -->
                <a href="{{ route('dashboard/home') }}" class="mobile-logo">
      ...
      <footer
        class="fixed bottom-0 left-0 w-full z-50 bg-white dark:bg-slate-800 text-slate-500 dark:text-slate-300 py-4 px-6 md:block"
        id="footer">
        <div class="site-footer ltr:ml-[248px] rtl:mr-[248px]">
          <div class="grid md:grid-cols-2 grid-cols-1 md:gap-5">
            <div class="text-center ltr:md:text-start rtl:md:text-right text-sm">
              COPYRIGHT © <span id="thisYear"></span> Parkomate Solutions LLP.
            </div>
          </div>
        </div>
      </footer>
