...
<div class="container">
    <h2 class="text-center">Table View Data</h2>
    @if ($errors->any())
            ...
            <table id="userTable" class="table table-hover table-bordered">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>{{getSitename($table->site_id)}}</td>
                        <td>{{ $table->table_name }}</td>
                        <td>
                            @if($can_edit == 1)
                            <form action="{{route('table.destroy', $table->table_id)}}" id="deleteForm-{{ $table->table_id }}" onsubmit="return confirmDelete(event, '{{ $table->table_id }}');" method="post">
                                @csrf
                                @method('DELETE')
                                <button type="submit" class="btn btn-outline-danger btn-sm">Delete</button>
                            </form>
                            <a href="{{ route('table.edit', $table->table_id) }}" class="btn btn-sm btn-outline-success ">Edit</a>
                            @endif
                        </td>
                    </tr>
                    @endforeach
            ...
            <form action="{{ route('table.store') }}" method="post">
                ...
                <div class="modal-body">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" name="table_name" class="form-control" id="table_name" value="{{ old('table_name') }}" required>
                    </div>
                </div>
